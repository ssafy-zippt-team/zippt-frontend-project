...
export async function fetchBoundary(level) {
  if (level >= 12) {
    if (cache.sido) return cache.sido;
    const { data } = await axios.get("/data/sidoSimple.geojson");
    return (cache.sido = data.features);
  }
  if (level >= 9) {
    if (cache.sigungu) return cache.sigungu;
    const { data } = await axios.get("/data/sigunguSimple.geojson");
    return (cache.sigungu = data.features);
  }
  ...
  const arrays = await Promise.all(fileNames.map((name) => axios.get(`/data/${name}`).then((r) => r.data.features)));
  return (cache.dong = arrays.flat());
}
